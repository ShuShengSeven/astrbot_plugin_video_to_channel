...
移植自 astrbot_plugin_parser（MIT）的 core/parsers/kuaishou.py，针对本插件
“仅搬运视频到腾讯频道”的场景做了适配：
- 解析快手页面中的 window.INIT_STATE；
- 只有 mainMvUrls（视频）时才创建 VideoContent；
- 快手图文/图集作品直接抛出 ParseException，不下载图片，避免浪费流量。
"""
...
class KuaiShouParser(BaseParser):
    ...
    async def _parse_v_kuaishou(self, searched: re.Match[str]):
        ...
        if not real_url:
            raise ParseException("快手链接重定向失败，无法获取作品页地址")
        ...
        async with self.session.get(real_url, headers=self.ios_headers) as resp:
            if resp.status >= 400:
                raise ParseException(f"获取快手页面失败 {resp.status}")
            response_text = await resp.text()

        pattern = r"window\.INIT_STATE\s*=\s*(.*?)</script>"
        matched = re.search(pattern, response_text)
        if not matched:
            raise ParseException("快手页面中未找到作品数据（window.INIT_STATE）")
        ...
        if photo is None:
            raise ParseException("快手页面中未包含视频或图集数据")

        video_url = photo.video_url
        if not video_url:
            raise ParseException("快手图文/图集暂不支持，本插件仅搬运视频")

        # 仅视频：创建视频内容（含封面），不创建图片内容
